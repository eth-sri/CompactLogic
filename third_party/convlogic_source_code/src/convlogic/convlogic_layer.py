...
class ConvLogicLayer(nn.Module):
    ...
    def __init__(
        ...
        :param num_chn:             Number of input channels each output channel samples from.
                                    0 means sample freely from all input channels in the group.
                                    Default is 2 (original behaviour).
        """
        ...
        if selection is None:
            selection = self.generate_selection(self.num_chn)
    ...
    def generate_selection(self, num_chn: int = 2) -> torch.Tensor:
        """
        Generate the selection tensor mapping each output channel to input positions.
        ...
        ch_sel = torch.empty((out_channels, num_inputs), dtype=torch.int32, device=dev)
        ...
        for gid, chans in enumerate(group_map):
            ...
            if cnt == 0:
                continue
            k = chans.numel()
            # num_chn == 0 means use all channels in the group
            n = k if num_chn == 0 else min(num_chn, k)

            # Pre-select n channels per output channel (with replacement from the group)
            chosen = torch.stack(
                [chans[torch.randint(0, k, (cnt,), device=dev)] for _ in range(n)], dim=1
            )  # (cnt, n)

            # For each of the num_inputs positions, pick one of the n pre-selected channels
            picks = torch.randint(0, n, (cnt, num_inputs), device=dev, dtype=torch.int64)
            for pos in range(num_inputs):
                ch_sel[idxs, pos] = chosen[torch.arange(cnt, device=dev), picks[:, pos]]
        ...
        # MY_COMMENT: packed shape (out_channels, num_inputs)
        return packed
